...
export default function App() {
  ...
  const loadDir = useCallback(async (dirId) => {
    ...
    try {
      const response = dirId ? await api.getDir(dirId) : await api.getRootDir();

      console.log("DIR RESPONSE:", response);

      const dir = response?.data || response;

      setItems(dir?.children || []);
    } catch (err) {
  ...
  async function handleNewFolder() {
    ...
    try {
      ...
      if (!pid) {
        ...
        pid = root?.data?.id || root?.id || root?.directoryId;
      }
  ...
  async function handleUpload() {
    ...
    input.onchange = async (e) => {
      ...
      try {
        ...
        const currentFolder = path.length > 0 ? path[path.length - 1].id : null;
    ...
    >
      ...
      >
        ...
        <Toolbar
          ...
          onRefresh={() =>
            loadDir(path.length > 0 ? path[path.length - 1].id : null)
          }
          loading={loading}
        />
      ...
      {modal?.type === "delete" && (
        ...
        >
          ...
          >
            <ModalBtn label="Cancelar" onClick={closeModal} secondary />
            <ModalBtn
              label={modalLoading ? "Deletando..." : "Deletar"}
      ...
      {modal?.type === "newFolder" && (
        <Modal title="Nova Pasta" onClose={closeModal}>
          ...
          >
            <ModalBtn label="Cancelar" onClick={closeModal} secondary />
            <ModalBtn
              label={modalLoading ? "Criando..." : "Criar"}
